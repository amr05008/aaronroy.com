...
| 7b | Image captions | A visible caption is an italic line directly under the image (no blank line between): `![alt](...)` then `*caption*` — `.prose img + em` in `src/styles/global.css` styles it. Don't hand-write `<figure>/<figcaption>` HTML in new posts. Every post image already gets a 1px gray border from `.prose img` — don't add one to the image file |
...
| 10 | Internal links | **Absolute, with a trailing slash**: `https://aaronroy.com/<slug>/`. Not root-relative — `src/pages/rss.xml.ts` only absolutizes `/images/`, `/og-images/` and `/videos/`, so a root-relative post link ships to feed readers as `href="/<slug>/"` and resolves against *their* domain. Missing the trailing slash costs a 308 and wastes the crawl signal |
| 10b | Link back into the archive | Does this post naturally reference something Aaron wrote before? If so, link it — and prefer an **older** post. As of 2026-08-08, 29 of 38 published posts had zero inbound links from any other post, and posts reachable only via the sitemap are measurably likelier to go uncrawled. Recent posts already link to each other; the pre-2024 archive is what's stranded. Suggest, never invent: verify the target exists before linking it. No natural fit → record "skipped: no natural link" in the gate report |
